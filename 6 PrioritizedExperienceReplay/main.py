...
from DDQNwithPER import DuelDQNAgent
...
def plot_rewards(rewards, title="Training rewards"):
    ...
    plt.plot(
        np.convolve(rewards, np.ones(15) / 15, mode="valid"), label="Smoothened rewards"
    )
    plt.xlabel("Episode")
...
if __name__ == "__main__":
    ...
    agent = DuelDQNAgent(env, update_target_freq=600)
    # print(agent.device)
    ...
    train_rewards = train_dqn(env, agent, num_episodes=1000, max_steps=300)
    ...
    plot_rewards(test_rewards, title="Duel DQN Testing rewards")
    env.close()
