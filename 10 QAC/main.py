...
from QAC import QACAgent, PolicyNetwork, QNetwork
...
def save_model(policy_model, q_model, path_policy, path_q):
...
def load_model(path_policy, path_q, input_dim, output_dim, hidden_dim=[128]):
    policy_model = PolicyNetwork(input_dim, output_dim, hidden_dim)
    q_model = QNetwork(input_dim, output_dim, hidden_dim)
    policy_model.load_state_dict(torch.load(path_policy))
    q_model.load_state_dict(torch.load(path_q))
    policy_model.eval()
    q_model.eval()
...
def main():
    ...
    save_model(
        agent.policy_net, agent.q_net, "models/qac_policy.pth", "models/qac_q.pth"
    )
    env.close()
